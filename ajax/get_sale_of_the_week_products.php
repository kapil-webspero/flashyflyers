...
$totalLimit = $limit = 4;
$page = $page_id;
$query = "`is_sale_of_the_week` = '1' AND `Status` = '1' ";
$adjacents = 2;
$pagination = "";
...
<script>(function(d, s, id) {
  ...
  fjs.parentNode.insertBefore(js, fjs);
    ...
    if($lastpage > 1)
    {
        ...
        //previous button
        if ($page > 1)
            $pagination.= '<a class="prev-page bx-shadow" onclick="return sale_of_the_week_pagged('.$prev.');"><img src="images/arrow-left.png" alt=""></a><ul>';
        else
        ...
        if ($lastpage < 7 + ($adjacents * 2))
        {
            for ($counter = 1; $counter <= $lastpage; $counter++)
            {
                if ($counter == $page)
                    ...
                    $pagination.= '<li><a onclick="return sale_of_the_week_pagged('.$counter.');">'.$counter.'</a></li>';
            }
        }
        elseif($lastpage > 5 + ($adjacents * 2))
        {
            if($page < 1 + ($adjacents * 2))
            {
                ...
                $pagination.= '...';
                $pagination.= '<li><a onclick="return sale_of_the_week_pagged('.$lpm1.');">'.$lpm1.'</a></li>';
                $pagination.= '<li><a onclick="return sale_of_the_week_pagged('.$lastpage.');">'.$lastpage.'</a></li>';
            }
            elseif($lastpage - ($adjacents * 2) > $page && $page > ($adjacents * 2))
            {
                $pagination.= '<li><a onclick="return sale_of_the_week_pagged(1);">1</a></li>';
                $pagination.= '<li><a onclick="return sale_of_the_week_pagged(2);">2</a><li>';
                $pagination.= '<li>...</li>';
        ...
        //next button
        if ($page < $counter - 1)
            $pagination.= '</ul><a onclick="return sale_of_the_week_pagged('.$next.');" class="next-page bx-shadow float-right"><img src="images/arrow-right.png" alt=""></a>';
        else
