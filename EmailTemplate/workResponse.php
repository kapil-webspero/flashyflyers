...
$adminEmail = 'support1@yopmail.com';
$to = $Email;
...
if(!empty($designerData)){
	...
	$email->addTo($designerData['Email']);
	$email->addContent("text/html", $DesingermailContent);
...
$email1->setFrom($from);
$email1->setSubject($Adminsubject);
$email1->addTo($adminEmail);
$email1->addContent("text/html", $adminMailContent);
...
try {
		...
		// print_r($response->headers());
		// print $response->body() . "\n";
		echo "Your mail has been sent successfully to Admin.";
} catch (Exception $e) {
		// echo 'Caught exception: '. $e->getMessage() ."\n";
		echo 'Unable to send email. Please try again.';
}
...
$email2->addTo($to);
$email2->addContent("text/html", $customerMailContent);
