...
$getProduct = array("Title","slug", "Subtitle", "Category", "ProductType", "Addon", "Description", "2D", "3D", "Static", "Motion", "Animated", "Defaultsizes", "OtherSizes", "3dTextEditable", "Baseprice", "Tags", "Status" );
...
if(false){
    ...
    if(isset($_REQUEST['updateproduct'])) {
        ...
        if(count($_REQUEST['tags'])>0) {
            $prod_tags = implode(",",$_REQUEST['tags']);
        }
        if($threedav == "yes") {
            $threed = 1;
        } else {
        ...
        UpdateRcrdOnCndi(PRODUCT, "`Title` = '$name',`slug` = '$slug', `Subtitle` = '$subtitle', `Category` = '$product_cate', `ProductType` = '$product_type', `Addon` = '$addon_val', `Description` = '".addslashes($editor)."', `FullDescription` = '".addslashes($editor1)."', `2D` = '1', `3D` = '$threed', `Static` = '1', `Motion` = '$motion', `Animated` = '$animated', `Defaultsizes` = '$default_size', `OtherSizes` = '$other_size', `3dTextEditable` = '$edir_threed', `Baseprice` = '$price', `Status` = '$product_status', `Tags` = '$prod_tags',`is_seasonal` = '$is_seasonal',`is_sale_of_the_week` = '$is_sale_of_the_week',`is_regular` = '$is_regular',`parent_product_cat_id` = '".$_REQUEST['parent_product_types']."',`child_product_cat_id` = '".$_REQUEST['child_product_types']."',`CustomProduct` = '".$CustomProduct."',`CustomProductFiledsType` = '".$selectFiledsType."',`CustomeProductfieldsSettings` = '".$CustomeProductfieldsSettings."',`custom_pro_tooltip` = '".$custom_pro_tooltip."',`custom_pro_tooltip_msg` = '".$custom_pro_tooltip_msg."',`template_type` = '".$template_type."',`psd_price` = '".$psd_price."',`psd_file` = '".$file_name."'","`id` = '$productID'");
    ...
    if(isset($_REQUEST['createproduct'])) {
		     extract($_POST);
		$checkCodequery = "SELECT COUNT(*) as num FROM ".PRODUCT." WHERE Title = '".$name."'";
        ...
        $fn = array("Title","slug", "Subtitle", "Category", "ProductType", "Addon", "Description", "FullDescription", "2D", "3D", "Static", "Motion", "Animated", "Defaultsizes", "OtherSizes", "3dTextEditable", "Baseprice", "Tags", "Status" ,"is_seasonal", "is_sale_of_the_week", "is_regular", "product_addon_id","parent_product_cat_id","child_product_cat_id","CustomProduct","CustomeProductfieldsSettings","CustomProductFiledsType","custom_pro_tooltip","custom_pro_tooltip_msg");
        $fv = array($name,$slug, $subtitle, $product_cates, $product_type, $addon_val, addslashes($editor), addslashes($editor1), '1', $threed, '1', $motion, $animated, $default_size, $other_size, $edir_threed, $price, $prod_tags, 1, $is_seasonal, $is_sale_of_the_week, $is_regular,$product_addon_id,$_REQUEST['parent_product_types'],$_REQUEST['child_product_types'],$CustomProduct,$CustomeProductfieldsSettings,$selectFiledsType,$custom_pro_tooltip,$custom_pro_tooltip_msg);
        $productID = InsertRcrdsGetID($fn, $fv, PRODUCT);
        $_SESSION['NEWPRODUCTID'] = $productID;
			...
			$slug = checkproductSlug(PRODUCT,'ID',$slug,$productID);
				...
				UpdateRcrdOnCndi(PRODUCT, "`slug` = '$slug',template_type='".$template_type."',psd_price='".$psd_price."',psd_file='".$file_name."'","`id` = '$productID'");

        $curl = curl_init();
        curl_setopt_array( $curl, array(CURLOPT_RETURNTRANSFER => 1,
                                   CURLOPT_URL            => SITEURL . 'EmailTemplate/new_product_notification.php?productId=' . $productID,
                                   CURLOPT_USERAGENT      => 'Curl Test') );
        $resp = curl_exec( $curl );
        curl_close( $curl );

        echo "<script> window.location.href = '".ADMINURL."create-product-2.php';</script>";
        exit();
    }}
}
    ...
    <div class="container">
        <div class="main-content pl-60 pr-60 bx-shadow">
            <div class="page-head mb-4">
                ...
                <?php if(isset($_SESSION['NEWPRODUCTID']) && !empty($_SESSION['NEWPRODUCTID'])) { ?>
                    <a href="create-product-2.php">View product media
            ...
            <div class="row">
                        ...
                        {									
                            if($par_cat_array[$i]['ID'] == $_REQUEST['parent_product_types'])
                                $selected = "selected";
                            else
                                $selected = "";
                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="name" onChange="changeSlug(this.value)" onKeyUp="changeSlug(this.value)"  class="form-control" placeholder="Please enter title/name" value="<?=$_REQUEST['name'];?>" required>
                    </div>
                    <div class="col-md-6">
                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="subtitle" class="form-control" placeholder="Please enter subtitle" value="<?=$_REQUEST['subtitle'];?>">
                    </div>
                    <div class="col-md-6">
                            <label>Product Status</label>
                            <select class="form-control" name="product_status">
                                <option value="1" <?php if($_REQUEST['product_status'] == 1) { echo "selected"; } ?>>Active</option>
                                <option value="0" <?php if($_REQUEST['product_status'] != 1) { echo "selected"; } ?>>Inactive</option>
                            </select>
                        </div>
                ...
                <div class="row">
                    <div class="col-md-6">
                           ...
                           <label style="display:block;" class="custom-control custom-radio">
                            <input type="radio" class="custom-control-input template_type" value="regular_template" name="template_type" <?php if($_REQUEST['template_type'] == 'regular_template' || $_REQUEST['template_type']=="") { echo "checked"; } ?>>
                            <span class="custom-control-indicator"></span>
                            <span class="custom-control-description">Regular Template</span>
                        </label>
                        <label style="display:block;" class="custom-control custom-radio">
                            <input type="radio" class="custom-control-input template_type" value="psd_customize_template" name="template_type" <?php if($_REQUEST['template_type'] == 'psd_customize_template') { echo "checked"; } ?>>
                            <span class="custom-control-indicator"></span>
                            <span class="custom-control-description">PSD & Customize Template</span>
                        </label>
                         <label style="display:block;" class="custom-control custom-radio">
                            <input type="radio" class="custom-control-input template_type" value="psd_only" name="template_type" <?php if($_REQUEST['template_type'] == 'psd_only') { echo "checked"; } ?>>
                            <span class="custom-control-indicator"></span>
                    ...
                    <span class="custom-file-control"></span>
                                      ...
                                      <div class="outputFilename"></div>
                                        ...
                                        <input type="hidden" name="psd_file_hidden" id="psd_file_hidden" value="">					
                    </div>
                        </div>
                <div class="row">
                ...
                <div class="col-md-6 psd_customized_block" style="display:none;">
                            ...
                            <input type="text" name="psd_price"  class="form-control" placeholder="Please enter psd price" value="<?=$_REQUEST['psd_price'];?>">
                        </div>
                   <div class="col-md-6 priceBox">
                        <label>Product Price</label>
                        <input type="text" name="price" class="form-control" placeholder="Please enter base price" value="<?=$_REQUEST['price'];?>" required>
                    </div>
                    <?php /*?><div class="col-md-6">
                        ...
                        foreach($categoryList as $categoryData) { ?>
                            <label class="custom-control custom-checkbox">
                                <input type="checkbox" name="product_cate[]" class="custom-control-input" value="<?=$categoryData['cat_id'];?>">
                                <span class="custom-control-indicator"></span>
                    ...
                    <?php /*?><div class="col-md-6">
                            ...
                            <select id="tags" name="tags[]" class="form-control" multiple>
                                ...
                                <option value="Other Cover">Other Cover</option>
                            </select>
                        </div><?php */?>
                </div>
                ...
                <div class="TagsBlock">
                    ...
                    <select id="productTags" name="productTags[]" multiple>
                        ...
                        <option value="<?php echo $singleTag['Id'] ?>" <?php if(in_array($singleTag['Id'],$_REQUEST['productTags'])){ ?> selected='selected' <?php } ?>><?php echo $singleTag['TagName'] ?></option>
                        <?php } ?>
                    </select>
                        <?php }  else{ ?>
                ...
                <?php /*?><div class="mb-4">
                    ...
                    <label class="custom-control custom-radio">
                        <input type="radio" class="custom-control-input" value="yes" name="threedav">
                        <span class="custom-control-indicator"></span>
                        <span class="custom-control-description">Yes</span>
                    </label>
                    <label class="custom-control custom-radio">
                        <input type="radio" class="custom-control-input" value="no" name="threedav" checked>
                        <span class="custom-control-indicator"></span>
                ...
                <div class="mb-4">
                    ...
                    <br>
                    <label class="custom-control custom-checkbox">
                        <input type="checkbox" class="custom-control-input" value="motion" name="feature[]">
                        <span class="custom-control-indicator"></span>
                        <span class="custom-control-description">Motion</span>
                    </label>
                    <?php /*?><label class="custom-control custom-checkbox">
                        <input type="checkbox" class="custom-control-input" value="animated" name="feature[]">
                        <span class="custom-control-indicator"></span>
                ...
                <div class="mb-4">
                    ...
                    <br>
                    <label class="custom-control custom-radio">
                        <input type="radio" class="custom-control-input" value="yes" name="threededit">
                        <span class="custom-control-indicator"></span>
                        <span class="custom-control-description">Yes</span>
                    </label>
                    <label class="custom-control custom-radio">
                        <input type="radio" class="custom-control-input" value="no" name="threededit" checked>
                        <span class="custom-control-indicator"></span>
                        <span class="custom-control-description">No</span>
                    </label>
                ...
                <div class="row CustomProductFiledsSetting" <?php if($getProduct['Addon'] == 1) { echo 'style="display:none;"'; } ?>>
                    ...
                    <div class="col-md-6">
                        ...
                        <label class="custom-control custom-radio" style="margin-bottom: 40px;">
                            <input type="radio" class="custom-control-input" value="yes" name="addon">
                            <span class="custom-control-indicator"></span>
                            <span class="custom-control-description">Yes</span>
                        </label>
                        <label class="custom-control custom-radio">
                            <input type="radio" class="custom-control-input" value="no" name="addon" checked>
                            <span class="custom-control-indicator"></span>
                            <span class="custom-control-description">No</span>
                        </label>
                    </div>
                    <div class="col-md-6" id="addonCategory" style="display:none;">
                        ...
                        <select class="form-control" name="product_category_addon">
                            ...
                            foreach($addonCategoryList as $addonCategoryData) { ?>
                                <option value="<?=$addonCategoryData['ID'];?>" <?php if($addonCategoryData['ID']==$_REQUEST['product_category_addon']){ ?>  selected <?php } ?>><?=$addonCategoryData['category_name'];?></option>
                            <?php } ?>
                        </select>
                    </div>
                </div>
                <div class="row show_flyer_section customeFilesHideShowBase">
                    <div class="col-md-6">
                        <label>Show flyer section</label>
                        <br>
                        <select name="show_flyer[]" multiple="multiple" class="select-flyer-section form-control">
                            <option value="is_regular" <?php if(in_array('is_regular',$_REQUEST['show_flyer'])){ ?>  selected <?php } ?> <?php if(empty($_REQUEST['show_flyer'])){ ?> selected <?php } ?>>Regular</option>
                            <option value="is_seasonal" <?php if(in_array('is_seasonal',$_REQUEST['show_flyer'])){ ?>  selected <?php } ?>>Seasonal</option>
                            <option value="is_sale_of_the_week" <?php if(in_array('is_sale_of_the_week',$_REQUEST['show_flyer'])){ ?>  selected <?php } ?>>Sale of the week</option>
                ...
                <?php if(!empty($addonProductList) && count($addonProductList) > 0){ ?>
                    <div class="row customeFilesHideShowBase" id="product_addon" style="display:none;">
                        <div class="col-md-6">
                            <label>Addons: </label>
                            <br>
                            <select name="product_addon_id[]" multiple="multiple" class="select-addon-product form-control">
                                <?php foreach ($addonProductList as $addonProductKey => $addonProductValue ){?>
                                    <option value="<?php echo $addonProductValue['id']; ?>" <?php if(in_array($addonProductValue['id'],$_REQUEST['product_addon_id'])){ ?>  selected <?php } ?> ><?php echo $addonProductValue['Title']; ?></option>
                                <?php } ?>
                ...
                <div class="mb-4 size-opt">
                    ...
                    foreach($getProdSizes as $keySize1 => $mainSize) { ?>
                        <label class="custom-control custom-checkbox">
                            <input type="checkbox" name="default_size[]" <?php if(in_array($keySize1,$_REQUEST['default_size'])){ ?>  checked <?php } ?> class="custom-control-input  defaultSize" value="<?=$keySize1;?>">
                            <span class="custom-control-indicator"></span>
                ...
                <div class="mb-4 size-opt">
                    ...
                    foreach($getProdSizes as $keySize2 => $otherSize) { ?>
                        <label class="custom-control custom-checkbox">
                            <input type="checkbox" name="other_size[]" class="custom-control-input otherSizeCheck otherSizes other_<?=$keySize2;?>"  <?php if(in_array($keySize2,$_REQUEST['other_size'])){ ?>  checked <?php } ?> value="<?=$keySize2;?>">
                            <span class="custom-control-indicator"></span>
                ...
                <button type="submit" name="createproduct" class="form-btn-grad btn-block">Save</button>
            </form>
        </div>
