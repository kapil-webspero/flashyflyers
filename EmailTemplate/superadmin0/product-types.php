<?php
	...
	if($_REQUEST['parent_type'] != 0)
	{
		...
		$path .=$_REQUEST['name'];	
			...
			if($_REQUEST['CustomeProductfieldsSettings']!="" && !empty($_REQUEST['CustomeProductfieldsSettings'])){
					$CustomeProductfieldsSettings = serialize($_REQUEST['CustomeProductfieldsSettings']);
			}
			
			
			
			InsertRcrdsByData(PRODUCT_TYPE,"`parent_id` = '".$_REQUEST['parent_type']."',`Price` = '".$price."',`design_labor_cost` = '".$design_labor_cost."',`fieldsSettings` = '".$fieldsSettings."', `Name` = '".$_REQUEST['name']."', `Slug` = '".sanitize_title_with_dashes($_REQUEST['name'])."', `path` = '".$path."'");
	...
	$fieldsSettings= "";
			...
			$path .=$_REQUEST['name'];	
			UpdateRcrdOnCndi(PRODUCT_TYPE, "`parent_id` = '".$_REQUEST['parent_type']."', `fieldsSettings` = '".$fieldsSettings."', `Name` = '".$_REQUEST['name']."', `Slug` = '".sanitize_title_with_dashes($_REQUEST['name'])."', `path` = '".$path."'","`ID` = '".$_REQUEST['ID']."'");
...
$fieldsSettings =$fieldsSettingsCustomProduct= array();
$ParentIDType = 0;
if($_REQUEST['mode']=="edit" && $_REQUEST['ID']>0){
	...
	$CustomeProductfieldsSettings  = $rs_types['CustomeProductfieldsSettings'];
	if(!empty($fieldsSettings)){
		$fieldsSettings = unserialize($fieldsSettings);	
	}
	
	if(!empty($CustomeProductfieldsSettings )){
		$fieldsSettingsCustomProduct = unserialize($CustomeProductfieldsSettings );	
	}
        ...
        <div class="container">
            <div class="main-content bx-shadow pl-60 pr-60">
                 ...
                 <label class="childTypeData">Child type name</label>
                        ...
                        <div class="col-md-6">
                           ...
                           <label class="custom-control custom-checkbox">
                                ...
                                <span class="custom-control-indicator"></span>
                                <span class="custom-control-description"> Notes to graphic designer</span>
                            </label>
                            </li><?php */?>
                           ...
                           <?php /* 
                            <div class="CustomProductFileds">
                            
                            	<h4>Custom Product Fields Settings</h4>
                                <?php 
                            ...
                            <li>
                            <label class="custom-control custom-checkbox">
                                <input type="checkbox" name="CustomeProductfieldsSettings[<?php echo $single['id']; ?>]" <?php if (!empty($fieldsSettingsCustomProduct) && array_key_exists($single['id'],$fieldsSettingsCustomProduct)){ echo "checked";} ?> class="custom-control-input" value="1">
                                <span class="custom-control-indicator"></span>
                ...
                <div class="table-responsive">
                    <table class="table sorting users-table1 table-1">
                        <thead>
                            <tr>
                             <th scope="col" onClick="window.location='product-types.php?<?php echo http_build_query($_GET); ?>&key_order=ID&key_order_by=<?php echo $ordeBy; ?>'" class="header <?php if($key_order=="ID" && $key_order_by=="DESC"){echo "headerSortUp";} ?> <?php if($key_order=="ID" && $key_order_by=="ASC"){echo "headerSortDown";} ?>">#ID</th>
                                <th scope="col" onClick="window.location='product-types.php?<?php echo http_build_query($_GET); ?>&key_order=Name&key_order_by=<?php echo $ordeBy; ?>'" class="header <?php if($key_order=="Name" && $key_order_by=="DESC"){echo "headerSortUp";} ?> <?php if($key_order=="Name" && $key_order_by=="ASC"){echo "headerSortDown";} ?>">Parent type</th>
	...
	?>                   
        <tr>
          ...
          <td><?php echo ($showData['parent_id']>0)?categoryNameFromId($showData['parent_id']):$showData['Name']; ?></td>
            ...
            <td class="data-view"><a href="?ID=<?=$showData['ID'];?>&mode=edit" class="view">Edit</a>
            <?php if( $_SESSION['userType'] == "admin"){ ?>
            <div class="data-delete"> <a href="?deleteID=<?=$showData['ID'];?>" onClick="return confirm('Are you sure you want to perform this action?')"><i class="fas fa-trash-alt"></i></a></div>
            </td>
