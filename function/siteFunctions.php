<?php
	...
	function GetSglDataOnCndiWithOrderBy($tName, $cndy, $Field,$OrderFeild, $OrderType ){
		$query = mysql_query("SELECT ".$Field." FROM ".$tName." WHERE ".$cndy ." ORDER BY ".$OrderFeild." ".$OrderType." LIMIT 1") or die(mysql_error());
		$row = mysql_fetch_assoc($query);
		return $row;
	}
	...
	function allDesignTransactions($ID) {
  	...
  	function getProdSizeArr() {
		...
		foreach($sizeList as $sizeArr) {
			$sizeNArr[$sizeArr['ID']] = array("id" =>$sizeArr['ID'], "name" => $sizeArr['Size'], "price" => $sizeArr['Price'], "image_path" => $sizeArr['image_path']);
		}
		return $sizeNArr;
	}
...
function getTurnArroundTypeByID($id){
	 $array = array("1"=>'12 Hours Same-Day',"2"=>'24 hours',"3"=>'2-3 business days',"4"=>'3-5 business days','5'=>'1-2 days','6'=>'2-3 days');
...
function getCustomProductActiveTags()
{
	 $getProducts = GetMltRcrdsOnCndi(PRODUCT, " CustomProduct='yes' and Addon='0'");
		$productsTagsArray = array();
		
		if(!empty($getProducts)){
			foreach($getProducts as $singleProduct){
			if(!empty($singleProduct['Tags'])){
				$productsTagsExplded = explode(",",$singleProduct['Tags']);
				if(!empty($productsTagsExplded)){
					foreach($productsTagsExplded as $single){
						$productsTagsArray[$single] = $single;	
					}	
				}	
			}	
			}
		...
		$db_cat_rs = array();
		if(!empty($productsTagsArray)){
		$query = mysql_query("SELECT * FROM ".PRODUCT_TAGS." where TagStatus='Active' and Id IN('".implode("','",$productsTagsArray)."') order by TagName asc");
...
function getCustomProductSettingFormFields($parent_id,$ID){
		...
		$ReturnArray = array();
		if(!empty($row) && !empty($row['CustomeProductfieldsSettings'])){
			$fieldsSettings = unserialize($row['CustomeProductfieldsSettings']);
			$ReturnArray = $fieldsSettings;	
		}
	return	$ReturnArray;	
...
function getRelatedApproveImages($productID){
	...
	$tableOrder = ORDER;
		...
		while($row2 = mysql_fetch_assoc($query)) {
				...
				$query1 = mysql_query("SELECT AttachmentThumb FROM ".$tableChangeReq." WHERE  ResponseState='1' and ProductID='".$productID."' and Attachment!='a:0:{}' and OrderID='".$row2['Id']."' order by ID desc limit 1") or die(mysql_error());
				while($row = mysql_fetch_assoc($query1)) {
				
				
				$Attachment = unserialize($row['AttachmentThumb']);
				$ext= strtolower(pathinfo($Attachment[0]['large'], PATHINFO_EXTENSION));
				if(!empty($Attachment) && ($ext=="jpg" || $ext=="jpeg" || $ext=="png"  || $ext=="webp"  || $ext=="gif" )){
						$data['large'][] = $Attachment[0]['large'];
						$data['small'][] = $Attachment[0]['small'];
					}
				}
		}
...
function multidimation_to_single_array($array) { 
  if (!is_array($array)) { 
    return false; 
  } 
  $result = array(); 
  foreach ($array as $key => $value) { 
    if (is_array($value)) { 
      $result = array_merge($result, multidimation_to_single_array($value)); 
    } else { 
...
function getTimerCounterHours($timer){
	if($timer==""){
		return 0;	
	}

/*$rs = GetMltRcrdsWthSmFldsOnCndi(ORDER, "TransactionID='".$orderID."'", "*");
	if(!empty($rs)){
		foreach($rs as $single){
			if($single['customeProductsFileds']==""){
				$productTimerArray[$single['TurnAroundTime']] = $single['TurnAroundTime'];	
			}
			if($single['customeProductsFileds']!=""){
				$customeProductsFileds = multidimation_to_single_array(unserialize($single['customeProductsFileds']));
				
				$productTimerArray[$customeProductsFileds['turnaround_time']] = $customeProductsFileds['turnaround_time'];	
			}
		}	
	}*/
...
$deliArr = array(0=>0,1 => "12", 2 => "24", 3 => "72","4"=>"24","5"=>"72","6"=>"120");
$hoursArray = array();
$totalHours = 120;

/*if(!empty($productTimerArray)){
foreach($productTimerArray as $v){
	$hoursArray[$deliArr[$v]] = $deliArr[$v];	
}
arsort($hoursArray);
if(!empty($hoursArray)){
	$totalHours = key($hoursArray);	
}		
}
*/
		
	 return $deliArr[$timer];
}
function getAllCustomer(){
		$cutomers =	GetMltRcrdsOnCndiWthOdr(USERS, "Status!='inactive' and UserType='user'", "FName", "asc") ;
		return $cutomers;		
}

function getCustomerByID($userId){
		$cutomers =	GetMltRcrdsOnCndiWthOdr(USERS, "Status!='inactive' and UserID='".$userId."' and UserType='user'", "FName", "asc") ;
			$single = $cutomers[0]; 
			if($single['FName']!=""){
...
function getSaleOrderByKey($key,$userId,$type="customer"){
		if($type=="sales_rep"){
			$return =	GetSglRcrdWthSmFldsOnCndi(TRANSACTION, "RepKey='".$key."' and PaymentStatus='Sales pending payment' and RepUserID='".$userId."'", '*');	
		}else{
			
			$return =	GetSglRcrdWthSmFldsOnCndi(TRANSACTION, "RepKey='".$key."' and (PaymentStatus='Sales pending payment' or PaymentStatus='Pending payment') and UserID='".$userId."'", '*');
		}
		return $return;		
}


function createRandomChar( $length = 8 ) {
...
function imageResize($imageResourceId,$width,$height) {
	...
	imagealphablending($targetLayer, true);
	imagesavealpha($targetLayer, true);
	$bgcolor = imagecolorallocatealpha($targetLayer, 0, 0, 0, 127);
	imagefill($targetLayer, 0, 0, $bgcolor);
    imagecopyresampled($targetLayer,$imageResourceId,0,0,0,0,$targetWidth,$targetHeight, $width,$height);


    return $targetLayer;
}
function ImageThumbCreate($sourceProperties,$FileExt,$file,$fileNewName,$folderPath,$digitalOceanFolder=""){
	  $imageType = $sourceProperties[2];
        ...
        switch ($imageType) {
            ...
            case IMAGETYPE_PNG:
                ...
                imagepng($targetLayer,$folderPath. $fileNewName. "_thum.". $FileExt);
                break;
            ...
            case IMAGETYPE_GIF:
                ...
                $targetLayer = imageResize($imageResourceId,$sourceProperties[0],$sourceProperties[1]);
                imagegif($targetLayer,$folderPath. $fileNewName. "_thum.". $FileExt);
                break;
            ...
            case IMAGETYPE_JPEG:
                ...
                $targetLayer = imageResize($imageResourceId,$sourceProperties[0],$sourceProperties[1]);
                imagejpeg($targetLayer,$folderPath. $fileNewName. "_thum.". $FileExt);
		...
		$image_data       = file_get_contents($folderPath.$fileNewName."_thum.". $FileExt); // Get image data
		file_put_contents( $folderPath.$fileNewName."_thum.webp", $image_data );
		
		digitalOceanUploadImage($folderPath.$fileNewName."_thum.".$FileExt,$digitalOceanFolder);
		digitalOceanUploadImage($folderPath.$fileNewName."_thum.webp",$digitalOceanFolder);
				
				
		return  $fileNewName. "_thum.". $FileExt;;
       // move_uploaded_file($file, $folderPath. $fileNewName. ".". $FileExt);
   //     echo "Image Resize Successfully.";
    
}


function ProductImageResize($imageResourceId,$width,$height,$heighSet=354) {

	
    $targetHeight =$heighSet;
...
function ProductImageThumbCreate($sourceProperties,$FileExt,$file,$fileNewName,$folderPath,$heighSet,$digitalOceanFolder=""){
	  ...
	  $imageType = $sourceProperties[2];
        ...
        switch ($imageType) {
            ...
            case IMAGETYPE_PNG:
                $imageResourceId = imagecreatefrompng($file); 
                $targetLayer = ProductImageResize($imageResourceId,$sourceProperties[0],$sourceProperties[1],$heighSet);
                imagepng($targetLayer,$folderPath. $fileNewName.".".$FileExt);
                break;
            ...
            case IMAGETYPE_GIF:
                $imageResourceId = imagecreatefromgif($file); 
                $targetLayer = ProductImageResize($imageResourceId,$sourceProperties[0],$sourceProperties[1],$heighSet);
                imagegif($targetLayer,$folderPath. $fileNewName.".".$FileExt);
                break;
            ...
            case IMAGETYPE_JPEG:
                $imageResourceId = imagecreatefromjpeg($file); 
                $targetLayer = ProductImageResize($imageResourceId,$sourceProperties[0],$sourceProperties[1],$heighSet);
                imagejpeg($targetLayer,$folderPath. $fileNewName.".". $FileExt);
		...
		$image_data       = file_get_contents($folderPath.$fileNewName.".". $FileExt); // Get image data
		file_put_contents( $folderPath.$fileNewName.".webp", $image_data );
		digitalOceanUploadImage($folderPath.$fileNewName.".".$FileExt,$digitalOceanFolder);
		digitalOceanUploadImage($folderPath.$fileNewName.".webp",$digitalOceanFolder);
		
		return  $fileNewName.".". $FileExt;;
    
}
function getProductBannerImageSizeWebp($fileName,$id,$size){
	if($fileName==""){
		return "";
	}
	...
	$fileName = str_replace(".".$ext,"",$fileName);
		$CheckSize="";
	if($size=="354"){
		$CheckSize =$fileName."_245X354";
	}
	if($size=="568"){
		$CheckSize =$fileName."_389X568";
	}
		$path = "products/".$id."/";
		if(digitalOceanCheckFile($path.$CheckSize.".webp")){
			return digitalOceanGetFileImage($path.$CheckSize.".webp");
		}else if(digitalOceanCheckFile($path.$CheckSize.".".$ext)){
			return digitalOceanGetFileImage($path.$CheckSize.".".$ext);
		}else{
			return digitalOceanGetFileImage($path.$fileName.".".$ext); 
	}
		
}


function getProductBannerImageSize($fileName,$id,$size){
	if($fileName==""){
		return "";
	}
	$ext = pathinfo($fileName, PATHINFO_EXTENSION);
	...
	if($size=="568"){
		...
		$path = "products/".$id."/";
		if(digitalOceanCheckFile($path.$CheckSize.".".$ext)){
			return digitalOceanGetFileImage($path.$CheckSize.".".$ext);
		}else{
		return digitalOceanGetFileImage($path.$fileName.".".$ext);
	}
...
function getProductVideoUrl($fileName,$id){
	if($fileName==""){
		return "";
	}
		$path = "products/".$id."/";
		if(digitalOceanCheckFile($path.$fileName)){
			return digitalOceanGetFileImage($path.$fileName);
		}else{
		return "";
	}
...
function getFlayerBannerImageSizeWebp($fileName,$size){
	if($fileName==""){
		return "";
	}
	$ext = pathinfo($fileName, PATHINFO_EXTENSION);
...
function getFlayerBannerImageSize($fileName,$size){
	if($fileName==""){
		return "";
	}
	$ext = pathinfo($fileName, PATHINFO_EXTENSION);
...
function getOrderPreviewFile($fileName,$size="354"){
	if($fileName==""){
		return "";
	}
	$ext = pathinfo($fileName, PATHINFO_EXTENSION);
	...
	$path = SITE_BASE_PATH."uploads/work/".$CheckSize.".".$ext;
	
		if(file_exists($path)){
			return $CheckSize.".".$ext;
		}else if(file_exists(SITE_BASE_PATH."uploads/work/".$fileName."_thum.".$ext)){
			return $fileName."_thum.".$ext;
		}else{
			return $fileName.".".$ext; 
	}
...
function ProductImageThumbCreateForScript($sourceProperties,$FileExt,$file,$fileNewName,$folderPath,$heighSet){
	  $imageType = $sourceProperties[2];
		...
		$imageWebP=  imagecreatefromjpeg($folderPath. $fileNewName.".". $FileExt);
			ob_start();
			imagejpeg($imageWebP,NULL,100);
			$contWebP=  ob_get_contents();
			ob_end_clean();
		$WebPcontent =  imagecreatefromstring($contWebP);
		imagewebp($WebPcontent, $folderPath.$fileNewName.".webp");
...
function getOrderThumbFileWebp($fileName,$size="354"){
	if($fileName==""){
		return "";
	}
	$ext = pathinfo($fileName, PATHINFO_EXTENSION);
...
function getOrderThumbFile($fileName,$size="354"){
	if($fileName==""){
		return "";
	}
	$ext = pathinfo($fileName, PATHINFO_EXTENSION);
...
function FlayerImageSrc($imageName,$size ="354",$class=""){
 return '<picture>
  <source srcset="'.getFlayerBannerImageSizeWebp($imageName,$size ).'" type="image/webp">
  <source srcset="'.getFlayerBannerImageSize($imageName,$size ).'" type="image/jpeg">
<img '.$class.'  src="'.getFlayerBannerImageSizeWebp($imageName,$size ).'" width="374" height="550">
</picture>';
}


function productImageSrc($imageName,$productID,$size ="354",$class=""){
 return '<picture>
  <source srcset="'.getProductBannerImageSizeWebp($imageName,$productID,$size ).'" type="image/webp">
  <source srcset="'.getProductBannerImageSize($imageName,$productID,$size ).'" type="image/jpeg">
<img '.$class.'  src="'.getProductBannerImageSizeWebp($imageName,$productID,$size ).'" width="375" height="550">
</picture>';
}


function OrderImageSrc($imageName,$size ="354",$class=""){
 return '<picture>
  <source srcset="'.getOrderThumbFileWebp($imageName,$size ).'" type="image/webp">
  <source srcset="'.getOrderThumbFile($imageName,$size ).'" type="image/jpeg">
<img '.$class.'  src="'.getOrderThumbFileWebp($imageName,$size ).'">
</picture>';
}


function customProductImageURL($fileName){
 ...
 if($fileName==""){
	...
	$fileName = str_replace(".".$ext,"",$fileName);
		$path = "custom_product/";
		if(digitalOceanCheckFile($path.$fileName.".".$ext)){
			return digitalOceanGetFileImage($path.$fileName.".".$ext);
		}else{
			return "";
		}
...
function OrderImageURL($fileName){
...
function checkDiscountCode($chekType=""){

	session_start();
	if(isset($_SESSION['DISCOUNT_DATA'])){
		$code=$_SESSION['DISCOUNT_DATA']['discountData']['DiscountName'];
...
function digitalOceanUploadDir($directory,$name){
require_once(SITE_BASE_PATH."digitalocean/spaces.php");
$space = Spaces("OS7LKE6B7STN2XHS2UDE", "2ML7QAhfWEXBtZveB7uedZPR92J/EbBrtK0aS9nrVFw")->space("assets.media.flashyflyers", "nyc3");
$space->uploadDirectory($directory, $name,'',array('ACL'=>'public'));

}


function digitalOceanUploadImage($ImagePath,$digitalOceanUploadPath){
require_once(SITE_BASE_PATH."digitalocean/spaces.php");
$space = Spaces("OS7LKE6B7STN2XHS2UDE", "2ML7QAhfWEXBtZveB7uedZPR92J/EbBrtK0aS9nrVFw")->space("assets.media.flashyflyers", "nyc3");
$upload = $space->uploadFile($ImagePath, $digitalOceanUploadPath."/".basename($ImagePath), "public");
...
function digitalOceanDeleteImage($ImagePath){
require_once(SITE_BASE_PATH."digitalocean/spaces.php");
$space = Spaces("OS7LKE6B7STN2XHS2UDE", "2ML7QAhfWEXBtZveB7uedZPR92J/EbBrtK0aS9nrVFw")->space("assets.media.flashyflyers", "nyc3");

$space->deleteFile($ImagePath);

if(file_exists($ImagePath)){
	unlink($ImagePath);
}
}
function digitalOceanCheckFile($ImagePath){
require_once(SITE_BASE_PATH."digitalocean/spaces.php");
$space = Spaces("OS7LKE6B7STN2XHS2UDE", "2ML7QAhfWEXBtZveB7uedZPR92J/EbBrtK0aS9nrVFw")->space("assets.media.flashyflyers", "nyc3");
$fileExists = $space->fileExists($ImagePath);
return $fileExists;
}
function digitalOceanGetFileImage($path){

return  "https://nyc3.digitaloceanspaces.com/assets.media.flashyflyers/".$path;
}
