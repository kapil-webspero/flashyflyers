                                                ...
                                                <div class="flyer-img">
                                                    ...
                                                    <?php if($p['filetype'] == 'image') {
                                                        ...
                                                        if (strpos($p['filename'],'res.cloudinary.com') !== false){
                                                            ?>
                                                            <img src="<?=httpToSecure($p['filename'])?>" alt="<?php echo $product['Title']; ?>">
                                                            <?php
                                                        }
                                                        else{
                                                            ?>
                    ...
                    <div class="col-lg-7 col-md-12">
                        <div class="product-summary bx-shadow pl-5 pr-5 pt-4 pb-5">
							...
							if(strstr(strtolower($_SERVER['HTTP_USER_AGENT']), 'mobile') || strstr(strtolower($_SERVER['HTTP_USER_AGENT']), 'android')) {
								...
								if($product['CustomeProductfieldsSettings']!=""){
									 ?>	
                                     
                            <form method="post" name="addon_form" class="customeFiledsProducts" style="margin-top:20px">
									<?php 
                                    $CustomeProductfieldsSettings = unserialize($product['CustomeProductfieldsSettings']);
									foreach($CustomeProductfieldsSettings as $singleFiledsKey=>$singleFiledsValue){
									?>
                                    <label><?php echo $customeProductFields[$singleFiledsKey]['label'] ?></label>
                                   
                                    <?php if($customeProductFields[$singleFiledsKey]['type']=="datepicker"){ ?>
                                    	<input type="text" onkeydown="if (event.keyCode == 13) {jQuery('.addon_buy').trigger('click'); return false; }"   autocomplete="off" name="customeProductFields[<?php echo $singleFiledsKey; ?>]"  class="form-control mb-3 datepicker">
                                    <?php } ?>
                                    
                                   
                                    <?php if($customeProductFields[$singleFiledsKey]['type']=="text"){ ?>
                                    	<input type="text" onkeydown="if (event.keyCode == 13) {jQuery('.addon_buy').trigger('click'); return false; }"  name="customeProductFields[<?php echo $singleFiledsKey; ?>]"  class="form-control mb-3">
                                    <?php } ?>
                                    
                                    <?php if($customeProductFields[$singleFiledsKey]['type']=="textarea"){ ?>
                                    	<textarea  name="customeProductFields[<?php echo $singleFiledsKey; ?>]"  class="form-control mb-3"></textarea>
                                    <?php } ?>
                                       <?php if($customeProductFields[$singleFiledsKey]['type']=="file"){ ?>
                                    	  <div class="img-upload">
                                            <div class="box__input">
                                                <div class="dropzone dz-clickable" id="my-awesome-dropzone-gallery"><span>+ &nbsp;Click or drag you photo here to upload</span>
                                                    <div class="BarSingle"></div>
                                                    <input type="file" class="cmb2-upload-file regular-text" name="product_gallery" id="product_gallery" value="" size="45" multiple>
                                        ...
                                        <div id="cwi_upload_image_preview_gallery"> </div>
                                    <?php } 
						...
						?>
                            ...
                            <form  method="post" name="addon_form" style="margin-top:20px; margin-bottom:30px;">
...
.customeFiledsProducts label{ font-size:18px; font-weight: 700;color: rgba(32, 32, 32, 0.8);
    ...
    .dropzone { background: rgba(0, 0, 0, 0.03) none repeat scroll 0 0; border: 1px solid rgba(0, 0, 0, 0.03); border-radius: 3px; display: table-cell; height: 200px; padding: 23px; position: relative; text-align: center; vertical-align: middle; color: #333; cursor: pointer; }
    #product_gallery { height: 100%; left: 0; opacity: 0; position: absolute; top: 0; width: 100%; z-index: 10000; cursor: pointer; }
    .dropzone span { font-size: 18px; color: #333; font-weight: 700; }
    .BarSingle { width: 1%; height: 10px; background-color: #377FEA; display: none; margin-top: 55px; }
    .cmb2-upload-button.button-secondary { display: none; }
    .cwi_image_outer { border: 1px solid #ededed; padding: 5px; width: 14%; position: relative; cursor: pointer; margin-top: 20px; margin-right: 25px; float: left; box-sizing: border-box; }
    .cwi_image_outer .remove-this {
	position: absolute;
	right: -12px;
	top: -8px;
