        ...
        foreach ( $value as $single_key => $single_value ) {
				...
				if($single_value['Attachment']!=""){
					...
					foreach($Attachment as $single){
						...
						if((float)$file_size < (float)FILE_SIZE_MB){	
						 $upload = do_uploads(getcwd()."/".$dynamic_dir."/".$single,$filename_arr[0]);
						 unlink(getcwd()."/".$dynamic_dir."/".$single);
						}else{
							$upload = 	$single;	
						}
				...
				if($single_value['AttachmentThumb']!=""){
					...
					foreach($AttachmentThumb as $single){
						...
						if((float)$file_size < (float)FILE_SIZE_MB){	
							$uploadThumb = do_uploads(SITE_BASE_PATH."/".$dynamic_dir.$single, $filename_arr[0]);
							unlink(SITE_BASE_PATH.$dynamic_dir.$single);
						}else{
							$uploadThumb = 	$single;	
						}
...
<div class="page-wrap bubble-bg-1">
    <div class="container">
        <div class="thankyou-msg bx-shadow">
            <h1 class="udr-heading">Order has been successfully <?php echo (!empty($_SESSION['RepKeyModify']))? "updated":"created"; ?>.</h1>
               <a href="<?= ADMINURL . 'orders.php' ?>" class="btn-grad btn-lg" style="margin-bottom:15px;">View
                         orders</a>
...
<div class="page-wrap bubble-bg-1">
    <div class="container">
        <div class="thankyou-msg bx-shadow">
            <h1 class="udr-main-eading">Thanks For Your Bussiness!</h1>
            <h2 class="udr-sub-heading">Your Order</h1>
                                ...
                                foreach($cart1 as $key2=>$cart){
                                    foreach($cart as $key => $product ) {
                                        ...
                                        <div class="cart-row pt-4 pb-4" id="cart_id_<?= $key2."_".$getProduct['id'];?>">
                                            ...
                                            {
                                                ?>
                                                <img src="<?=httpToSecure($getBanners['filename']);?>" alt="<?=$getProduct['Title'];?>">
                                                <?php
                                            }
                                            else
                                            {
                                                ?>
                                                <img src="uploads/products/<?=$getProduct['id'];?>/<?=$getBanners['filename'];?>" alt="<?=$getProduct['Title'];?>">
                                                <?php
                                            }
                                            ?>
                                            ...
                                            <div class="cart-price text-right">
                                                $<?php echo formatPrice(MEDIA_CHANGE_PRICE); $totalAmount += MEDIA_CHANGE_PRICE;?>
                                            </div>
                                        </div>
                                        <?php
                                    }}
