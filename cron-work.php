			...
			foreach($singleOrderID as $singleProductID){
				if(!empty($singleProductID)){
					...
					if(!empty($singleSize)){
						...
						//	echo  "Oendig".SITEURL.'EmailTemplate/RivisionComplated.php?OID='.$singleSize2['OrderID'];
							//echo "<br>";
						$curl = curl_init();
						...
						UpdateRcrdOnCndi(CHANGE_REQ,"`ResponseState` = '1'", $whereClasue);

						UpdateRcrdOnCndi(ORDER,"`ResponseState` = '1'", "`id` = '".$singleSize2['OrderID']."'");
...
//one days mail
$selectOneDays = ExecCustomQuery("SELECT * FROM `".CHANGE_REQ."` where OneDaysMail='No' and ResponseState!='1' and CreationDate <'".$OneDays."'");
