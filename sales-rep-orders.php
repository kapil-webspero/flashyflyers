<?php
	...
	if(isset($_REQUEST['orderUID']) && !empty($_REQUEST['orderUID'])) {
		...
		echo "<script> window.location.href = '".SITEURL."sale-rep-order-details.php?order_id=".$orderUID."';</script>";
		exit();
	}	
	
	if(isset($_GET['deleteorder']) && !empty($_GET['deleteorder'])) {
		$orderid 		= $_GET['deleteorder'];
		DltSglRcrd(REPRESENTATIVE_ORDERS, "`RepOrdeID` = '$orderid'");		
...
.order_status_success{ color:#3bc178 !important; text-transform:capitalize;}
.order_status_pending{ color:#0070c0!important;text-transform:capitalize;}
...
<body class="MyOrderPage">
	...
	$searchQuery = "`RepUserID` = '$AccessID' AND ";	
...
if($total_pages>0) {
	...
	?>                   
                                ...
                                <div style="display:inline-block; float:left;">
                                	...
                                	<a style="background: #0070c0;padding: 5px 18px;color: #fff;border-radius: 10px;text-decoration: none; display:block; margin-bottom:5px;display:block;" href="<?=SITEURL;?>sale-rep-order-details.php?order_id=<?=$showData['RepOrdeID'];?>" class="view">View</a>
								
                                
                                                                        <a class="finish-btn" style="background: #3bc178 ;padding: 5px 18px;color: #fff; display:block;border-radius: 10px;text-decoration: none;" href="<?php echo SITEURL; ?>cart.php?RepKeyModify=<?=$showData['RepKey'];?>">Edit</a>
                                    ...
                                    <td><?=date('M d, Y',strtotime($showData['RepAddedDate']))."<br>".date('g:i A',strtotime($showData['RepAddedDate']));?></td>
                                      <td><?php if(!empty($showData['RepCustomerID'])) { echo $usersArr[$showData['RepCustomerID']]['FName']." ".$usersArr[$showData['RepCustomerID']]['LName']."<br> ".$usersArr[$showData['RepCustomerID']]['Email']; } else { echo "Not Available"; }?></td>
                                    <td><?=$productNameData;?></td>
                                       <td class="order_status_<?=$showData['OrderStatus'];?>"><?=$showData['OrderStatus'];?></td>
